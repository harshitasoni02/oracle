...
def _store_official_prediction(
    metal: str,
    timeframe: str,
    generated_at,
    target_date,
    ...
    """
    Store the first 1d/1w prediction generated for each UTC calendar day.
    Later dashboard refreshes update Prediction but leave this record unchanged.
    """
    if timeframe not in ('1d', '1w'):
        return

    from oracle.models import OfficialPrediction

    OfficialPrediction.objects.get_or_create(
        metal=metal,
        timeframe=timeframe,
        prediction_day=generated_at.date(),
        defaults={
            'prediction_date': generated_at,
            'target_date': target_date,
            'previous_price_usd': round(current_price_usd, 2),
            'predicted_usd': float(pred['predicted_usd']),
            'predicted_direction': pred.get('direction') or 'sideways',
...
def generate_predictions(metal: str):
    ...
    for tf in ['1d', '1w', '2w']:
        ...
        target_date = generated_at + delta
        _store_official_prediction(
            metal=metal,
            timeframe=tf,
